...
> `shadowcast ablate`, `shadowcast diagnose` and `shadowcast inspect`.
...
**65.8%.** The synthetic figure is 98.4%. This is the number that matters and it is
published as it stands.

| | Agreement | False positive | False negative |
|---|---|---|---|
| Real match, `12_22/batch_001:0` | **65.83%** | 8.24% | **25.92%** |
| Synthetic, same code | 98.43% | 1.18% | 0.40% |
...
The failure is almost entirely **false negatives** — we claim darkness the game did not
have. The stream's own fog events say champions are visible **75.3%** of the time; our
masks produce **50.7%**.

### The gap decomposes into two separate problems

Measured by conditioning on how far each tick is from a *labelled* position anchor, where
the champion's location is known rather than interpolated:

| Distance from an anchor | Visible, median distance to nearest source | Hidden, median | Visible with **no source in range** |
|---|---|---|---|
| Within 0.5 s | 642 u | 1,512 u | **23.9%** |
| 2–5 s | 1,003 u | 1,621 u | 34.8% |
| Over 10 s | 1,991 u | 1,031 u | **65.3%** |

**Trajectory error dominates at long anchor gaps.** Past ten seconds from an anchor the
relationship *inverts* — hidden champions sit nearer a vision source than visible ones —
which means the position estimate carries no information there. Real anchors arrive
331–483 times per champion per match, but unevenly.

**A missing-source floor of 23.9% remains even with exact positions.** A quarter of the
moments the game calls visible have no modelled source within a champion's 1,350-unit
sight radius. Two known causes, neither yet fixed:

- **Lane minions are not in the stream we read.** They arrive via `BarrackSpawnUnit`
  (1,363 packets a match), which the packet seam does not carry — `SpawnMinion` contains
  only wards, camps, plants and ability summons. Substituting the deterministic wave
  schedule instead recovers 0.7 points, so the clump model is also too coarse: a real wave
  is six units spread over hundreds of units and pushes past the midpoint when it is
  winning, while the model is one disc parked at the lane centre.
- **Turret positions were recovered from attack packets**, which found only **6 of 24** —
  most turrets never fire inside a truncated twelve-minute window, and a turret with no
  position grants no vision. Now taken from `sr.TURRETS` by name, which matches 22 of 24
  exactly. Worth 1.2 points.

Neither turrets nor minions is the dominant term. The honest summary is that real fog
agreement is 65.8%, that roughly a quarter of the gap is missing sources and the rest is
trajectory error, and that the synthetic 98.4% was measuring a much easier problem.
